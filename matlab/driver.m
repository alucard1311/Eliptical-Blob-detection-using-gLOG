...
zPos = repmat(1.57, len, 1);
src_coords_zpos = horzcat(src_coords,zPos);
disp("Src with Z")
disp(src_coords_zpos);
disp("Src");
disp(src_coords);
...
imageWithBlobs = originalImg;

for i = 1:size(src_coords, 1)
    imageWithBlobs = insertShape(imageWithBlobs, 'FilledCircle', [src_coords(i, :), radii(i)], 'Color', 'red');
end
%%%%%%%%%
...
birdseye_view = imwarp(originalImg, tform, 'OutputView', imref2d(output_size));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function H_3D = compute_3D_homography(src, dst, height)
    N = size(src, 1);
    A = zeros(3 * N, 12);

    for i = 1:N
        x = src(i, 1); y = src(i, 2); z = src(i, 3);
        u = dst(i, 1); v = dst(i, 2); h = height(i);
